...
import com.uni.uni_erp.dto.erp.sales.StoreListDTO;
...
public interface StoreRepository extends JpaRepository<Store, Integer> {
    ...
    @Query("SELECT new com.uni.uni_erp.dto.store.StoreDTO(s.id, s.name) FROM Store s WHERE s.user.id = :userId")
    ...
    @Query("SELECT new com.uni.uni_erp.dto.erp.sales.StoreListDTO(s.id, s.name, s.is24Hours, s.isOpen, s.createdAt, u.name) FROM Store s JOIN s.user u")
