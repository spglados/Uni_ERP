...
import com.uni.uni_erp.domain.entity.erp.sales.Sales;
...
public interface SalesRepository extends JpaRepository<Sales, Integer> {
    ...
    @Query("SELECT new com.uni.uni_erp.dto.erp.sales.SalesQuantityDTO(sd.itemCode,sd.quantity,s.salesDate) " +
    ...
    @Query("SELECT new com.uni.uni_erp.dto.erp.sales.SalesDataDTO(YEAR(s.salesDate), SUM(s.totalPrice)) " +
            "FROM Sales s " +
            "GROUP BY YEAR(s.salesDate) " +
            "ORDER BY YEAR(s.salesDate)")
    List<SalesDataDTO> findTotalPriceByYear();

    @Query("SELECT new com.uni.uni_erp.dto.erp.sales.SalesDataDTO(MONTH(s.salesDate), SUM(s.totalPrice)) " +
            "FROM Sales s " +
    ...
    @Query("SELECT new com.uni.uni_erp.dto.erp.sales.SalesDataDTO(DAY(s.salesDate), SUM(s.totalPrice)) " +
            "FROM Sales s " +
    ...
    @Query("SELECT new com.uni.uni_erp.dto.erp.sales.MostProductSaleQuantityDTO(sd.itemCode, sd.itemName, SUM(sd.quantity)) FROM Sales s JOIN FETCH SalesDetail sd ON s.orderNum = sd.orderNum WHERE s.storeId = :storeId AND FUNCTION('DATE', s.salesDate) = :today GROUP BY sd.itemCode ORDER BY SUM(sd.quantity) DESC limit 5")
    List<MostProductSaleQuantityDTO> findMostProductSaleQuantityByStoreId(Integer storeId, LocalDate today);

    @Query("SELECT new com.uni.uni_erp.dto.erp.sales.SalesInfoDTO(" +
